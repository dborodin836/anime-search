...
def detail(request, anime_id, current_page=1):
    # If user sends request with negative page number
    # I shouldn't do it, this bug can be fixed in JS
    if current_page < 1:
        current_page = 1
    ...
    # raw_data = json.loads(response.text)
    episodes_response = requests.get(f'https://kitsu.io/api/edge/anime/{anime_id}/episodes?page[offset]={page_offset}')
    raw_ep_data = json.loads(episodes_response.text)

    raw_data = json.loads(response.text)

    results = raw_ep_data['meta']['count']  # Amount of the found results
    last_page = ceil(results / 10) - 1
